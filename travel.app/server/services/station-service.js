...
let stationMap = null;
let codeToNameMap = null;
let lastFetchTime = 0;
const CACHE_DURATION = 24 * 60 * 60 * 1000;

async function fetchStationData() {
  const now = Date.now();
  if (stationMap && now - lastFetchTime < CACHE_DURATION) {
    return stationMap;
  }
  ...
  try {
    ...
    stationMap = map;
    codeToNameMap = codeMap;
    lastFetchTime = now;
    console.log(`站点数据加载完成，共 ${Object.keys(map).length} 个站点`);
    return stationMap;
  } catch (error) {
    console.error('获取12306站点数据失败:', error.message);
...
function getCodeToNameMap() {
  return codeToNameMap || {};
}

module.exports = { fetchStationData, getFallbackStations, getCodeToNameMap };
